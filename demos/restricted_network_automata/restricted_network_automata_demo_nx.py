...
if __name__ == '__main__':
    ...
    def topology_rule(ctx):
        curr_map = ctx.network
        new_map = ctx.network.copy()
        for i in underlying_network.nodes:
            in_degree_i = curr_map.in_degree(i)
            for j in underlying_network.nodes:
                if i == j: continue
                in_degree_j = curr_map.in_degree(j)
                ...
                if combined_in_degrees == 2 and not curr_map.has_edge(j, i) and underlying_network.has_edge(j, i):
                    new_map.add_edge(j, i)
                # a link will survive if the combined degree of the two nodes it connects is 3
                elif combined_in_degrees == 3 and curr_map.has_edge(j, i):
                    pass
                # a link dies if it exists
                else:
                    if curr_map.has_edge(j, i): new_map.remove_edge(j, i)
        return new_map
