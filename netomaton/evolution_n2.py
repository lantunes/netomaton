...
class TopologyContext_N2(object):
...
class PerturbationContext_N2(object):
    """
...
class UpdateOrder_N2(Enum):
    ACTIVITIES_FIRST = 1
    TOPOLOGY_FIRST = 2
    SYNCHRONOUS = 3


def evolve_n2(network, initial_conditions=None, activity_rule=None, timesteps=None, input=None, topology_rule=None,
              perturbation=None, past_conditions=None, update_order=UpdateOrder_N2.ACTIVITIES_FIRST, copy_network=True,
    ...
    while True:
        ...
        if update_order is UpdateOrder_N2.ACTIVITIES_FIRST:
        ...
        elif update_order is UpdateOrder_N2.TOPOLOGY_FIRST:
        ...
        elif update_order is UpdateOrder_N2.SYNCHRONOUS:
            # TODO create test
            curr_network = evolve_topology(topology_rule, t, prev_activities, prev_network,
                                           trajectory, copy_network, persist_network)
            # added and removed nodes are ignore in this case
...
def do_activity_rule(t, inp, curr_activities, prev_activities, network, activity_rule, past, perturbation):
    ...
    for node_label in network.nodes:
        ...
        ctx = NodeContext_N2(node_label, t, prev_activities, neighbour_labels, neighbourhood_activities,
        ...
        if perturbation is not None:
            pctx = PerturbationContext_N2(node_label, curr_activities[node_label], t, node_in)
...
def evolve_topology(topology_rule, t, activities, prev_network, trajectory, copy_network,
    ...
    if topology_rule:
        if copy_network:
            network = network.copy()
        network = topology_rule(TopologyContext_N2(network, activities, t))
        if not network:
