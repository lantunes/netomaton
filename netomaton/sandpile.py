...
class Sandpile:
    def __init__(self, rows, cols, is_closed_boundary=True):
        ...
        self._adjacencies = cellular_automaton2d(rows=rows, cols=cols, neighbourhood="von Neumann")
    ...
    def activity_rule(self, ctx):
        if self._is_closed_boundary and ctx.cell_index in self._boundary_indices:
            return 0  # closed boundary conditions

        new_activity = ctx.current_activity

        neighbour_activities = list(ctx.activities)
        neighbour_activities.pop(ctx.neighbour_indices.index(ctx.cell_index))
