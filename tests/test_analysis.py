...
class TestAnalysis(RuleTest):
    ...
    def test_average_node_entropy_simple(self):
        activities = [[1, 0, 1],
                      [1, 1, 1],
                      [0, 0, 0],
                      [1, 0, 0]]
        ...
        np.testing.assert_almost_equal(avg_node_entropy, 0.8742, decimal=4)

    def test_average_node_entropy(self):
        activities = self._convert_to_matrix("rule30_random_init.ca")
        avg_node_entropy = ntm.average_node_entropy(activities.tolist())
        np.testing.assert_almost_equal(avg_node_entropy, 0.9946, decimal=4)
