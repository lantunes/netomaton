from netomaton import evolve_n2, WolframPhysicsModel_N2, Network
...
class TestWolframPhysicsModel(RuleTest):
    ...
    def test_init_unary_relation(self):
        ...
        model = WolframPhysicsModel_N2(config, rules)
    ...
    def test_network_to_config(self):
        ...
        self.assertEqual(config, WolframPhysicsModel_N2.network_to_config(network))
    ...
    def test_config_interconversion(self):
        ...
        network = WolframPhysicsModel_N2(expected, {}).network
        actual = WolframPhysicsModel_N2.network_to_config(network)
    ...
    def _evolve_wolfram_physics_model(config, rules, timesteps):
        model = WolframPhysicsModel_N2(config, rules)
        trajectory = evolve_n2(network=model.network, topology_rule=model.topology_rule, timesteps=timesteps)
