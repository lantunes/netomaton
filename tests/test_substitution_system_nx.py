...
class TestSubstitutionSystem(RuleTest):
    ...
    def _evolve_substitution_system(expected, rules):
        ...
        subn_system = ntm.SubstitutionSystem_NX(rules, len(initial_conditions))
        trajectory = ntm.evolve_nx(initial_conditions=initial_conditions, network=subn_system.network,
                                   activity_rule=subn_system.activity_rule, timesteps=rows)
        activities = {t: nx.get_node_attributes(n, "activity") for t, n in trajectory.items()}
