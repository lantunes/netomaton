from netomaton import evolve_nx, WolframPhysicsModel_NX
...
class TestWolframPhysicsModel(RuleTest):
    ...
    def test_init_unary_relation(self):
        ...
        model = WolframPhysicsModel_NX(config, rules)

        expected_network = nx.MultiDiGraph()
        expected_network.add_edge(1, 1, label="1", unary=True)
        self._assert_networks_equal(expected_network, model.network)
    ...
    def test_network_to_config(self):
        ...
        self.assertEqual(config, WolframPhysicsModel_NX.network_to_config(network))
    ...
    def test_config_interconversion(self):
        ...
        network = WolframPhysicsModel_NX(expected, {}).network
        actual = WolframPhysicsModel_NX.network_to_config(network)
    ...
    def _evolve_wolfram_physics_model(config, rules, timesteps):
        model = WolframPhysicsModel_NX(config, rules)
        trajectory = evolve_nx(network=model.network, topology_rule=model.topology_rule, timesteps=timesteps)
